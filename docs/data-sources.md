...
> 현재 상태: **연동 불가 (개인 자격).** 아래는 그 결론에 도달하기까지의 기록이다.

### 개인은 워크넷 채용정보 API를 받을 수 없다

경로가 둘로 보이지만 **끝은 한 곳**이다.

| 경로 | 실체 | 개인 사용 |
|------|------|----------|
| 고용24 직접 (`work24.go.kr`) | 실제 API 제공처 | **불가** |
| 공공데이터포털 `3038225` | **API 유형이 `LINK`** — 고용24로 보내기만 하는 카탈로그 등록 | **불가** (우회로 아님) |

고용24 직접 발급 쪽이 제공 데이터가 많아 먼저 시도했으나, 호출하면 거부된다.
...
함정은 **신청이 자동 승인되고 인증키까지 정상 발급된다는 점**이다. 신청현황에 키가 보이므로 다 된 것처럼 보이지만, 실제 호출에서 회원 등급으로 막힌다.

단서는 신청 화면에 있었다. `오픈API 채용정보 서비스 신청서` 다운로드 파일을 열어보면 제목이 `(민간 직업정보제공 사업자용)`이고 첨부로 **사업자등록증 · 직업정보제공사업신고확인증**을 요구한다. 채용정보 재배포는 직업안정법상 직업정보제공사업 신고 대상이라 개인에게는 열려 있지 않다.

공공데이터포털 `3038225`(`한국고용정보원_워크넷 채용정보`)를 우회로로 기대했으나 아니었다. 이 항목은 **`활용신청`이 아니라 `바로가기`만 제공하는 링크형 데이터셋**이라 포털 자체 엔드포인트도, `serviceKey` 접근도 없다.

**결론: 사업자 전환 전까지 워크넷은 연동 대상에서 제외한다.** 프로브에는 `worknet` 정의와 아래 호출 규약을 재시도용으로 남겨둔다.
...
- [ ] **대체 소스 확보** — 워크넷이 빠지면서 민간 공고 소스가 사람인 하나로 줄었다
  - [ ] 사람인 API 승인 후 응답 구조 분석 (현재 가장 중요한 경로)
  - [ ] 원티드 OpenAPI — 개인/비법인 사용 가능 여부 문의
  - [ ] 로켓펀치 Open API — 앱 등록 요건 확인
  - [ ] 공공데이터포털에서 `LINK` 유형이 아닌 채용 API 추가 조사
- [ ] 워크넷 재시도 조건 정리 — 사업자등록 + 직업정보제공사업 신고가 실제로 필요한지 확인
- [ ] 공고 상세 화면에 소스별 출처 표기 (UI 구현 시)

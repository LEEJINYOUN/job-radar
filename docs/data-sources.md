...
> 현재 상태: **명세 확인 완료, 실측 전.** 인증키 발급 후 `pnpm probe worknet`으로 응답 구조를 검증하고 이 절을 채운다.
...
- `callTp=D`(상세)가 따로 있다는 건 **목록에 본문이 없을 가능성**을 뜻한다. 사실이면 공고 1건당 추가 호출이 필요하므로 수집 비용 설계가 달라진다 — 실측 시 목록 응답에 본문 필드가 있는지 먼저 확인한다.
...
- [ ] 워크넷 인증키 발급 후 `pnpm probe worknet` — 2절의 실측 부분을 채운다
...
- [ ] 워크넷 두 경로(공공데이터포털 `3038225` vs 고용24 직접) 제공 필드 비교 후 하나로 정리

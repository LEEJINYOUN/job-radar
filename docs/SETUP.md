...
### 2-2. 고용24 Open-API (선택, 데이터 종류가 더 많음)

공공데이터포털을 거치지 않고 직접 발급받는 경로. 채용정보 외에 직업정보·훈련정보·고용통계까지 제공한다.

1. <https://www.work24.go.kr> 회원가입
2. **고객센터 → OPEN-API → Open-API 신청**
3. 승인 후 발급된 키를 `.env`의 `WORK24_API_KEY`에 입력

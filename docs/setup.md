...
### 2-2. 워크넷 채용정보 — 개인은 받을 수 없다 (신청하지 말 것)

시간을 쓰지 않도록 결론부터 적는다. **워크넷 채용정보 API는 개인 자격으로 연동할 수 없다.**

- 고용24(`work24.go.kr`) 직접 신청은 **자동 승인되고 인증키까지 발급되지만**, 호출하면
  `개인회원은 사용할 수 없는 OPEN-API입니다`로 거부된다
- 공공데이터포털 `3038225`는 우회로가 아니다. **API 유형이 `LINK`**라
  `활용신청` 없이 고용24로 보내기만 한다
- 채용정보 재배포에 직업안정법상 직업정보제공사업 신고(사업자)가 필요해서다

사업자 전환 시 재시도할 수 있도록 `pnpm probe worknet` 정의와
`WORK24_RECRUIT_KEY` / `WORK24_COMMON_CODE_KEY` 칸은 남겨뒀다.
경위와 호출 규약은 [data-sources.md](data-sources.md) 2절 참고.

...
| 소스 | 제공 형태 | 상태 |
|------|----------|------|
| 공공기관 채용정보 (잡알리오) | 공공데이터포털 | **연동 확인** — 누적 11만 건 |
| 사람인 채용정보 API | 승인 후 access-key | 승인 대기. 1일 500회 제한 |
| 원티드 OpenAPI | 인증키 신청 | 연동 검토 중 |
| 로켓펀치 Open API | 앱 등록 | 연동 검토 중 |
| 워크넷(고용24) 채용정보 | 고용24 OPEN-API | **보류** — 개인 자격 호출 불가 |
...
법적 제약만 있는 것도 아닙니다. **워크넷(고용24)은 공공 API인데도 개인이 쓸 수 없습니다.** 신청은 자동 승인되고 인증키까지 발급되지만 호출하면 `개인회원은 사용할 수 없는 OPEN-API입니다`로 막힙니다. 채용정보 재배포가 직업안정법상 직업정보제공사업 신고 대상이기 때문입니다.
